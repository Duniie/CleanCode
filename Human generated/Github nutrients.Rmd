...
```{r}
# FluxSpd <- function(Data = NutEro[NutEro$Speed != 0,], Sits, nut = c("NH4", "NO2", "NO3", "PO4", "DSi"), Data.acc = PP[,c(1:3,18:20)]) {
# 
...
```{r}
#write.csv(FluxSpeed, "Fluxes erosion per speed.csv")
FluxperSpeed <- read.csv("Fluxes erosion per speed.csv")[,-1] 
FluxperSpeed <- aggregate(FluxperSpeed[,6], by = list(Location = FluxperSpeed$Location, Nut = FluxperSpeed$Nut, Speed = FluxperSpeed$Speed), FUN = mean)
